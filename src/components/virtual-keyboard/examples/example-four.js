              ...
              <div className="card-body">
                <form
                  ...
                  onSubmit={e => {
                    ...
                    if (!isNaN(number1) && !isNaN(number2)) {
                      ...
                      if (answer) {
                        this.setState({ answer: Math.round(answer) || 'almost zero' }, () => {
                ...
                >
                  <input
                    ...
                    name="number1"
                    value={model.number1}
                    onFocus={e => this.setState({ currentInputElement: e.target })}
                    onChange={e => {
                      this.update('number1', e.target.value);
                    }}
                    placeholder="Enter number"
                  />
                  <select className="form-control mr-3 ml-3" value={model.action} onChange={this.handleChange}>
                ...
                {answer ? <div className="alert alert-info mt-2 text-center">Answer is: {answer}</div> : null}
              </div>
            </div>
          </div>
